...
rolę paveikti žmonių kasdienybei. Rašydami šią knygą esame vadinamojoje
**triukšmingų tarpinės skalės kvantinių technologijų** etape
(angl. *noisy intermediate-scale quantum*, trumpinys NISQ). NISQ
kvantiniai procesoriai turi ribotą (50--100) kubitų skaičių, pasižymi
sparčiai atsirandančiomis klaidomis ir nenaudoja klaidų taisymo
algoritmų. Tai neleidžia atlikti ilgų ir labai tikslių skaičiavimų.
...
kada persiklojusių bangų amplitudė vietomis padidėja ir sumažėja. Vis
dėlto panašumai tarp klasikinių ir kvantinių sistemų čia ir baigiasi.

Įsivaizduokime instrumento stygą, kuri gali vibruoti tik dviem
...
```{r dvieju-kubitu-supynimas, echo=FALSE, fig.cap="Dviejų kubitų kvantinis supynimas.", out.width='80%', fig.align='center'}
knitr::include_graphics("media/image1_1.png")
```
...
Antroji priežastis slypi neįveikiamų klasikinių užduočių skaičiavimuose
kvantiniu kompiuteriu. Jau dabar yra atrasta eksponentiškai laiko
išteklių atžvilgiu greitesnių kvantinių algoritmų už geriausius žinomus
klasikinius tam tikroms problemoms spręsti. Tarp jų yra Šoro pirminių
skaičių faktorizavimas ir diskrečiojo algoritmo apskaičiavimas. Tai
padaro klasikiniams kompiuteriams neįveikiamas problemas įveikiamas
kvantiniams. Taip pat yra atrasta esminių paspartinimų duomenų bazių
paieškos algoritmuose, tiesinių lygčių sprendinių paieškose; jie savo
ruožtu atveria platų problemų spektrą, kuriam galima taikyti kvantinę
kompiuteriją.
...
savybės, tokios kaip jos padėtis, judėjimo greitis ar energija, negali
būti tiesiogiai stebimi nepaveikiant pačios sistemos būsenos. Tai savo
ruožtu apibūdina tarsi paslėptą pasaulį, kurio nei tiesiogiai, nei
tiksliai pasiekti negalime. Kiek žinoma, gamta yra fundamentaliai
...
aplinkos kvantinėmis sistemomis, kurios klaidingai keičia jų santykines
...
taisymo principai ir algoritmai. Nors kvantiniai skaičiavimai turi
analoginį pobūdį, klaidos šiose sistemose gali būti skaitmeninamos ir
ištaisomos. Klaidų prevencijos ir taisymo metodai yra neatsiejama dalis
...
pačiais metais buvo pateiktas **Kurto Gödelio** (angl. *Kurt Gödel*) jo
...
```{r algoritmu-sudetingumas, echo=FALSE, fig.cap="Keletas algoritmų sudėtingumo pavyzdžių. Nuo sparčiausiai iki lėčiausiai augančio operacijų skaičiaus: eksponentinė elgsena, polinominė, logtiesinė ir tiesinė", out.width='80%', fig.align='center'}
knitr::include_graphics("media/image1_2.png")
```
...
paviršiaus įspaudimo gyliu ir deformacijomis. Bitas yra matematinis
objektas, turintis dvejetainę savybę, ir nepriklauso nuo jį
...
(\#eq:kubito-busena)
\end{equation}
Formaliai šis objektas leidžia apskaičiuoti rezultatų tikimybes
nustatant kvantinės sistemos būseną, kuri buvo paruošta taikant žinomą
procedūrų seką. Nei daugiau, nei mažiau. Kaip ir klasikinis bitas,
kubitas gali būti būsenoje 0 ($b=0$, nusakomoje
$|0\rangle$, būsenoje 1 ($a=0$,  nusakomoje
$|0\rangle$, tačiau kartu ir begalybėje kitų skirtingų
būsenų kombinacijų tolydžiai keičiant koeficientus $a$ ir $b$. Terminai
„kvantinė būsena" ir „vektorius" šiame kontekste dažnai vartojami
pakaitomis ir reiškia tą patį, nes kvantinė būsena yra visiškai nusakoma
vektoriumi. Vartojami skliausteliai $|\ldots\rangle$
indikuoja, kad šis objektas yra vektorius. Kubito būseną žymime savo
nuožiūra pasirinktu simboliu, šiuo atveju viršuje graikiška raide $\psi$
(tariama *psi*).

Vektorius $|\psi\rangle$ yra kitų dviejų vektorių,
...
$|\psi\rangle$ orientacija vektorių erdvėje. Baziniai
vektoriai turi svarbią savybę -- jie yra **tarpusavyje ortogonalūs**
(angl. *orthogonal*). Tai reiškia, kad kompleksinėje vektorių erdvėje
kampas tarp bet kurių dviejų bazinių vektorių yra statusis. Kvantinėje
kompiuterijoje tik būsenos, kurios yra ortogonalios, gali būti patikimai
atskirtos viena nuo kitos.

```{r blocho-sfera, echo=FALSE, fig.cap="Blocho sfera.", out.width='80%', fig.align='center'}
knitr::include_graphics("media/image1_3.png")
```

Koeficientai $a$ ir $b$, formaliai vadinami amplitudėmis, yra
kompleksiniai skaičiai (turi realiąją ir menamąją dalis). Amplitudės
gali kisti tolydžiai, tačiau yra tarpusavyje susietos taip, kad jų
...
skirtingą vektoriaus orientaciją, tačiau išlaiko vienetinį jo ilgį.
...
Sferos pusiaujas ($θ=90^{\circ}$) nusako visas $|0\rangle$ ir
$|1\rangle$ būsenų superpozicijas, kurios skiriasi viena
...
kurio taško sferos paviršiuje įvardija visas galimas kubito būsenas.
...
kad superpozicija suyra. Amplitudžių kvadratų vienetinė suma atspindi
tai, kad tikimybė $p$ rasti $|0\rangle$ arba
$|1\rangle$ būseną turi susidėti į 1, kitų variantų nėra.
...
Net ir paruošus identišką kubito būseną ir ją pamatavus begalę kartų,
koeficientų $a$ ir $b$ unikaliai įvardyti taip nepavyks. Žinosime tik jų
...
tačiau informacija apie ją kompleksiniame kvadrate ir matavimo proceso
metu yra prarandama.
...
būseną. Šis metodas bendrai reikalauja paruošti identišką kvantinę
būseną ir daugybę kartų atlikti sumaniai parinktus skirtingo tipo būsenų
matavimus. Tad kvantinių būsenų tomografija nėra įprastai taikoma
...
vadinama **„tylėk ir skaičiuok"** (angl. *shut up and calculate*).
...
identitetą. Antrojo tipo vartai išvestyje apverčia įvesties bito vertę
...
kuriame vieno bito galimos transformacijos yra tik **tapatybės
transformacija** (angl. *identity*) ir *NOT* loginiai vartai, skirtingų
...
NOT*, trumpinys $\mathrm{cX}$) kartu su bendro tipo 1 kubito vartais leidžia
realizuoti vieną galimą universalų rinkinį. Vartai $\mathrm{cX}$ apverčia
...
kubitų. Vienas pavyzdys $\mathrm{cX}$, veikiančių 2-kubitų būseną
$11\rangle$ tiesinėje algebroje:
\begin{equation}
\mathrm{cX}|11\rangle = \begin{bmatrix}
\begin{matrix}
1 & 0 \\
0 & 1 \\
...
(\#eq:cX-matrica)
\end{equation}
Tačiau kvantinėje kompiuterijoje galime pritaikyti $\mathrm{cX}$, kai
...
\begin{equation}
\mathrm{cX}(a|0\rangle + b|1\rangle)\otimes |0\rangle = a|00\rangle + b|11\rangle\,.
(\#eq:supynimas)
\end{equation}
...
```{r kvantine-grandine, echo=FALSE, fig.cap="Kvantinė grandinė.", out.width='80%', fig.align='center'}
knitr::include_graphics("media/image1_4.png")
```
...
pritaikytos fazės $\theta_1$ ir $\theta_2$. Kvantinės grandinės yra
dažniausiai užbaigiamos kubitų būsenų matavimu.
...
Kvantinių bitų registras. Registras -- tai kubitų rinkinys, kuriame
koduojama ir apdorojama informacija. Registrą galima inicializuoti į
mums žinomą pradinę būseną, standartiškai $|0\rangle$
visiems kubitams.

Universalių kvantinių loginių vartų rinkinys. Loginiai vartai atlieka
registro būsenų keitimo operacijas, leidžiančias koduoti ir apdoroti
informaciją.

Kubitų būsenų matavimai. Norint nuskaityti registre apdorotą
...
```{r kvantinio-skaiciavimo-procesas, echo=FALSE, fig.cap="Kvantinio skaičiavimo procesas.", out.width='80%', fig.align='center'}
knitr::include_graphics("media/image1_5.png")
```
...
principo sudėlioti ir optimizuoti „rankomis", tačiau to fiziškai tampa
nebeįmanoma padaryti procesoriui, turinčiam 1000 kubitų. Todėl
neišvengiamai teks automatizuoti kvantinių grandinių kompiliavimą. Tam
reikia atsižvelgti į esančius kvantinio kompiuterio suvaržymus norint
kuo efektyviau realizuoti norimą algoritmą. Artimuoju kvantinių
...
kontroliuoti kvantiniam procesoriui. Ši dalis iš esmės apima patį
fizinio kvantinio kompiuterio kūrimą ir priklauso nuo architektūros.
Kvantiniame fiziniame lygmenyje yra fizinių kubitų sistema. Ji izoliuota
...
Čia pasitelkiama klasikinė kompiuterija atlikti kvantinio procesoriaus
...
knygos rašymo metu 2 kubitų *CNOT* loginiai vartai, ribojantys bendrą
...
loginiam kubitui realizuoti gali prireikti $10^1$--$10^6$ fizinių kubitų,
...
```{r fazes-amplitudes-slopinimas, echo=FALSE, fig.cap="Fazės ir amplitudės slopinimas. Parodytos kreivės nusako, kokia tikimybė rasti kubitą pradinėje nurodytoje būsenoje bėgant laikui. Iliustracijai panaudota $T_1 = T_2 = 300$ mikrosekundžių dekoherencijos trukmė.", out.width='80%', fig.align='center'}
knitr::include_graphics("media/image1_6.png")
```
