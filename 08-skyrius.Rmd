...
Klasikinių bei kvantinių sistemų modeliavimas fizikoje yra pagrįstas
diferencialinių lygčių sprendinių paieška. Jie apibūdina
dinamiką laike bei erdvėje ir atspindi dėsnius, kuriais šios
...
pozicijų, elektromagnetinių ir gravitacinių laukų, atitinkamai, konfigūracijas bei kitimą. Tik itin speficinėse situacijose sprendinius galima rasti ir užrašyti žinomomis funkcijomis, tad yra pasitelkiami kompiuteriniai skaičiavimai. Įprastai, kompiuteriniai skaičiavimai diskretizuoja
...
Pavienių ir sudėtinių kvantinių sistemų dinamiką apibūdina 3 skyriuje
...
### Aizingo modelis {#ising-hamiltonian-coding}

Pirmiausia pradėkime nuo pastebėjimo, kad sudėtinių kvantinių sistemų modeliavimas,
...
kontroliuojamą eksperimentą su šiomis sistemomis. Tarp tokių yra elektronas, protonas ir neutronas, turintys $1/2$ **sūkį** (angl. *$1/2$ spin*). Šios dalelės magnetiniame lauke elgiasi panašiai kaip magnetiniai
...
pozicijoje yra po vieną nesuporuotą elektroną (kubitą).

**Aizingo modelio** (angl. *Ising model*) hamiltonianas leidžia apibūdinti sistemos,
...
suminėje dedamojoje nusako magnetines sąveikas tarp vienas šalia kito ($i$, $i + 1$)
...
$-jI\otimes Z\otimes Z\otimes I$. Antra dedamoji hamiltoniane
apibūdina išorinio magnetinio lauko išilgai $x$ ašies efektą kiekvienam
iš $n$ kubitų; veikdamas trečią kubitą 4 kubitų sistemoje
...
unitariojo operatoriaus $U = \mathrm{e}^{\frac{-\mathrm{i}Ht}{\hbar}}$ veiksmą
...
bendrai $n$ kubitų superpozicija. Toliau matysime vieną universalų metodą, kaip apskaičiuoti kubitų būsenos laiko evoliuciją kvantiniame kompiuteryje.
...
$s$ žingsnių, kuriame kiekvienas laiko intervalas trunka
...
(\#eq:laiko-diskretizacija-8)
\end{equation}
Naudodami pasirinktinai mažą žingsnį $\Delta t$ ir atlikdami
iteraciją $s$ kartų, apskaičiuosime norimą sistemos laiko evoliuciją
visame laiko intervale $t$. Viršuje parodyta pirmosios eilės $U$
...
$\Delta t$, pagal kurį laiko evoliucija yra strobuojama, taip pateikiant efektyvią seką:
\begin{equation}
|\psi(s\Delta t)\rangle
= U_s(\Delta t)\cdots U_3(\Delta t)U_2(\Delta t)U_1(\Delta t)|\psi(0)\rangle\,.
(\#eq:nuo-laiko-priklausantis-u-trot)
\end{equation}
...
Tam galime panaudoti \@ref(matematikos-skyrius) skyriuje
...
\end{bmatrix}\,.
(\#eq:z-times-z-eksponente)
\end{equation}
\@ref(eq:z-times-z-eksponente) lygybės tenzorinėje sandaugoje
...
$|10\rangle$) įgauna $\mathrm{e}^{-\mathrm{i}\theta}$. \@ref(fig:z-times-z-grandine) pav. parodyta grandinė,
realizuojanti $\mathrm{e}^{\mathrm{i}\theta Z_k\otimes Z_l}$ operatorių kubitams
$k_1$ ir $k_2$ pasitelkiant ancilą $a_1$.

```{r z-times-z-grandine, echo=FALSE, fig.cap="Loginė grandinė, realizuojanti $\\mathrm{e}^{\\mathrm{i}\\theta Z_1\\otimes Z_2}$ operatorių. Ancila kubitui pritaikomi $R_z$ posūkio loginiai vartai", out.width='52%', fig.align='center'}
knitr::include_graphics("media/image8_1.svg")
```
...
(\#eq:bendra-pradine-busena-ising)
\end{equation}
...
\end{aligned}
(\#eq:skirtingos-fazes-skirtingo-lyginumo-busenoms)
\end{equation}
...
2 kubitų būseną, veikiamą laiko evoliucijos operatoriumi $\mathrm{e}^{\frac{-\mathrm{i}H_1 t}{\hbar}}
= \mathrm{e}^{\mathrm{i}\theta Z_k\otimes Z_l}$:
\begin{equation}
...
(\#eq:pozicijos-operatorius)
\end{equation}
\@ref(eq:pozicijos-operatorius) lygybėje $\hat{x}$ operatorius, veikiantis vieną iš tikrinių
...
pilnumo savybę (\@ref(matematikos-skyrius) skyrius), atrodo taip:
\begin{equation}
|\psi\rangle = \int_{-\infty}^{\infty} |x\rangle\langle x|\psi\rangle\,dx
= \int_{-\infty}^{\infty}\psi(x)|x\rangle\,dx\,.
(\#eq:busena-per-erdvinius-bazinius-vektorius)
\end{equation}
...
(\#eq:tikimybe-rasti-erdves-intervale)
\end{equation}
...
(\#eq:schroedingerio-lygtis-banginei-funkcijai)
\end{equation}
...
(\#eq:hamiltonianas-kinetine-potencine)
\end{equation}
...
(\#eq:bangines-funkcijos-evoliucija)
\end{equation}
...
\mathrm{e}^{\frac{-\mathrm{i}V\Delta t}{\hbar}}\right)^s\,;(\#eq:e-h-dt-approx)\\
...
\mathrm{e}^{\frac{-\mathrm{i}V\Delta t}{\hbar}}\psi(x,0)\,.(\#eq:psi-x-t-approx)
\end{align}
...
\begin{align}
U_{\mathrm{FT}}\psi(x) = & \psi(p)\,;(\#eq:bangine-funkcija-impulso-atvaizdavime)\\
\psi(p) = & \frac{1}{\sqrt{2^n}}\sum_{x = 0}^{2^n - 1}
\psi(x) \mathrm{e}^{\mathrm{i}2\pi xp/2^n}\,.(\#eq:bangine-funkcija-impulso-per-pozicijos)
\end{align}
...
(\#eq:kinetines-energijos-veikimas)
\end{equation}
...
(\#eq:evoliucijos-iteracinis-algoritmas)
\end{equation}
...
\end{cases}\,.
(\#eq:energijos-barjeras)
\end{equation}
Kvantinė grandinė, perteikianti
$\theta = \frac{V(x)\Delta t}{\hbar}$ fazes
$\{|0\rangle , |3\rangle , |7\rangle\}$
būsenoms, parodyta \@ref(fig:potencines-energijos-grandine) pav.

```{r potencines-energijos-grandine, echo=FALSE, fig.cap="Loginė grandinė, atliekanti potencinės energijos funkcijos $V(x)$ su trimis barjerais veiksmą banginei funkcijai $\\psi(x)$ viename laiko intervale", out.width='54%', fig.align='center'}
knitr::include_graphics("media/image8_2.svg")
```
...
\@ref(kvantiniai-loginiai-vartai-grandines) skyriuje parodytu Tofoli
vartais pagrįstu metodu (žr. \@ref(bendro-tipo-salyginiai-vartai) poskyrį). Šitoks metodas yra bendro
...
(\#eq:kinetine-energija-loginiais-vartais)
\end{equation}
...
(\#eq:kinetine-energija-3-kubitams)
\end{equation}

Jau žinome, kaip kvantinėje grandinėje
loginiais vartais perteikti visus čia matomus narius. 

Šriodingerio lygties sprendime turime paruošti pradinę banginę funkciją $\psi(x,0)$.
...
įprastai gali būti efektyviai koduojamos. Pavyzdžiui, **Gauso
...
amplitudžių, yra pateikta \@ref(fig:gauso-diskretizuota-funkcija) pav. Kitas galimas būdas -- pasitelkti
...
```{r gauso-diskretizuota-funkcija, echo=FALSE, fig.cap="Gauso formos diskretizuota banginė funkcija, perteikta 16-os taškų gardelėje", out.width='86%', fig.align='center'}
knitr::include_graphics("media/image8_3.svg")
```
...
rasime visą **tikimybių tankio funkciją** $|\psi(x)|^2$
(angl. *probability density function*), kuri nusako tikimybes rasti
kvantinę sistemą kiekviename erdvės taške $x$.

Vietoj šio matavimo, pasitelkiant Hadamardo testą (žr. \@ref(hadamard-testas-poskyris) poskyrį), galima apskaičiuoti globalius banginės funkcijos
...
pavaizduoti \@ref(fig:klasikinio-ml-metodai) pav.


```{r klasikinio-ml-metodai, echo=FALSE, fig.cap="Klasikinio mašininio mokymosi metodai", out.width='88%', fig.align='center'}
knitr::include_graphics("media/image8_4.svg")
```
...
(\#eq:kvantines-atminties-orakulas)
\end{equation}
...
(žr. \@ref(duomenu-kodavimas) poskyrį), tad $N = 2^n$ dydžio duomenų bazė perteikiama tik
...
pavaizduotas \@ref(fig:kvantiniu-grandiniu-klasifikatorius) pav.


```{r kvantiniu-grandiniu-klasifikatorius, echo=FALSE, fig.cap="Kvantinėmis grandinėmis paremtas klasifikatorius. Loginių operacijų grandinė $A$ atlieka pradinės būsenos paruošimą; ji yra toliau apdorojama modelio $U(\\theta)$ grandinėje optimizuojant klasifikatorių. Galiausiai, atliekama binarinė klasifikacija išmatuojant pirmojo kubito būseną", out.width='46%', fig.align='center'}
knitr::include_graphics("media/image8_5.svg")
```
...
pavaizduotas \@ref(fig:hibridinis-ml-algoritmas) pav. Klasikinė modelio
mokymo dalis pavaizduota brūkšnine linija.

```{r hibridinis-ml-algoritmas, echo=FALSE, fig.cap="Hibridinis kvantinis-klasikinis mokymo algoritmas. Kvantinis įrenginys yra naudojamas išvesties ir gradientų suskaičiavimui; kvantinės grandinės parametrai $\\theta$ atnaujinami naudojantis klasikiniu algoritmu", out.width='61%', fig.align='center'}
knitr::include_graphics("media/image8_6.svg")
```
...
vartai yra bendriausio tipo $U_3 (\alpha,\beta,\gamma) \equiv G$
(žr. \@ref(vieno-kubito-vartai) poskyrį). O štai 2 kubitų sąlyginius $cG$ galime užrašyti:
\begin{equation}
cG = |0\rangle\langle 0|\otimes I + |1\rangle\langle 1|\otimes G\,.
(\#eq:salyginiai-diadomis-ml)
\end{equation}
...
vartai, kuriuose $j$-tasis kubitas yra adresatas, o
$(j + r)\mod(n)$-tas kubitas kontroliuojantis. Trijų kubitų
atveju, kai $r = 1$, toks blokas pavaizduotas \@ref(fig:triju-kubitu-blokas) pav.


```{r triju-kubitu-blokas, echo=FALSE, fig.cap="3 kubitų kvantinį supynimą sudarančios grandinės blokas. $G_j$ yra 1 kubito loginiai vartai", out.width='58%', fig.align='center'}
knitr::include_graphics("media/image8_7.svg")
```
...
(\#eq:tankio-matrica-tikriniais-vektoriais)
\end{equation}
...
\@ref(eq:a-kaip-unitarusis-op) lygtimi. Operatoriaus $U$ tikrines vertes $\mathrm{e}^{\mathrm{i}\lambda_k t}$ randamos
panaudojant kvantinį fazės nustatymo algoritmą,
aprašytą \@ref(kvantinis-fazes-nustatymas) poskyryje.

Algoritme reikia apskaičiuoti matricos $\rho$ eksponentę. Šį skaičiavimą paspartina
paprastas matematinis triukas pasitelkiant bet kokią pagalbinę
tankio matricą $\sigma$. Laiką $t$ padalijame į $s$ dalių, $t =
...
(\#eq:swap-eksponente-pca)
\end{equation}
kur $\mathrm{Tr}_P$ yra dalinis pėdsakas per pirmą kintamąjį, o $W$ -- $SWAP$
loginiai vartai. $SWAP$ operatoriaus eksponentė gali būti efektyviai realizuota
kvantinėse grandinėse, todėl, naudojantis \@ref(eq:swap-eksponente-pca) lygtimi,
galima efektyviai realizuoti ir tankio matricos eksponentės skaičiavimą.
