...
superpozicijos būsenose ilgą laiką. I skyriuje minėtos $T_1$ ir $T_2$
...
**informaciją** (angl. *redundant information*). Jeigu dalis
...
(angl. *gate fidelity*) siekia $\sim 10^{-4}$, tad klaidų taisymo algoritmai
yra pageidautini.
...
+ 2\mathrm{Re}\lbrack \langle e_1 | e_2 \rangle\rbrack\big)\,;\\
...
\begin{align}
U|0\rangle\otimes|e\rangle = & |0\rangle\otimes|e_1 \rangle + |1\rangle\otimes|e_2 \rangle\,; \\
U|1\rangle\otimes|e\rangle = & |0\rangle\otimes|e_3 \rangle + |1\rangle\otimes|e_4 \rangle\,.
\end{align}
...
```{r loginio-kubito-paruosimas, echo=FALSE, fig.cap='Loginio kubito būsenos $|\\psi\\rangle_L$ paruošimas naudojant 3 fizinius kubitus.', out.width='80%', fig.align='center'}
knitr::include_graphics("media/image9_1.png")
```
...
```{r bito-apvertimo-taisymo-grandine, echo=FALSE, fig.cap="Bito apvertimo klaidos aptikimą ir taisymą atliekanti grandinė.", out.width='80%', fig.align='center'}
knitr::include_graphics("media/image9_2.png")
```
...
Surinksime visą informaciją siekdami tai pademonstruoti. IV skyriuje
...
\end{equation}
9.4 pav. pateikiame grandinę,
iliustruojančią šio loginio kubito paruošimą.

![](./media/image9_4.png)

**9.4 pav.** Loginė grandinė, paruošianti loginį kubitą, skirtą taisyti fazės
klaidai.
...
aplinkos sistemos būsenų $|e_i\rangle$ ir pradinio
ancilos kubito būsenoje $|0\rangle$:
\begin{equation}
|\Psi\rangle = \sum_{E_i \in \varepsilon} E_i|\psi\rangle\otimes|e_i \rangle\otimes|0\rangle\,.
\end{equation}
...
\otimes\big(|000\rangle + |111\rangle\big)\nonumber\\
\equiv & |+\rangle\otimes|+\rangle\otimes|+\rangle\,;\\
...
$|-\rangle$. Grandinė atliekant kodo paruošimą pateikta
9.5 pav.

![](./media/image9_5.png)

**9.5 pav.** Loginė grandinė, paruošianti Šoro 9 kubitų kodo loginį kubitą
...
\begin{align}
|0\rangle_L : & \quad |000\rangle + |111\rangle\rightarrow |000\rangle - |111\rangle\,;\\
|1\rangle_L : & \quad |000\rangle - |111\rangle\rightarrow |000\rangle + |111\rangle\,.
\end{align}
...
\begin{align}
X_1\otimes X_2\otimes X_3\big(|000\rangle + |111\rangle\big ) = & |000\rangle + |111\rangle = |+\rangle\,;\\
X_1\otimes X_2\otimes X_3\big(|000\rangle - |111\rangle\big ) = & -\big(|000\rangle - |111\rangle) = - |-\rangle\,.
\end{align}
...
skyriaus 9.2 poskyryje pateikto įrodymo, kad visas klaidas galima
...
$\mathcal{P} = \mathcal{P}^{\otimes n}$. Galima parodyti, kad visi Pauli grupės
...
\begin{align}
K_1 = & X\otimes Z\otimes Z\otimes X\otimes I\,; \\
K_2 = & I\otimes X\otimes Z\otimes Z\otimes X\,; \\
K_3 = & X\otimes I\otimes X\otimes Z\otimes Z\,; \\
K_4 = & Z\otimes X\otimes I\otimes X\otimes Z\,.
\end{align}
...
\end{equation}
Neprarandant bendrumo, bet kokią $n$ kubitų būseną
...
projekciją į visų $n$ stabilizatorių bendrą +1 poerdvį bei +1 loginio
...
\begin{align}
|0\rangle_L = & \frac{1}{4}(I + K_1)\otimes(I + K_2)\otimes(I + K_3)\otimes(I + K_4)|00000\rangle\,;\\
|1\rangle_L = & \frac{1}{4}(I + K_1)\otimes(I + K_2)\otimes(I + K_3)\otimes(I + K_4)|11111\rangle\,.
\end{align}
Hadamardo testo kvantinė grandinė (žr. 6.7.1 poskyrį) su
$U \rightarrow K$ gali atlikti norimą projekciją taikant anksčiau minėtą
principą. Atkreipiame dėmesį, kad galutinė Hadamardo testo būsena
...
atsparus skaičiavimas** (angl. *fault-tolerant computation*) yra
...
jeigu 2 kubitų $CNOT$ loginiuose vartuose kontrolinis kubitas patyrė
...
efektas klaidų sklidime dėl VI skyriuje minėtos fazės atatrankos. Jeigu,
prieš atliekant $CNOT$ loginius vartus, įvyksta fazės klaida
...
loginiais elementais. Imkime kaip pavyzdį 9.6 pav. parodytą kvantinę
grandinę, sudarytą iš klaidoms neatsparių loginių elementų, atliekančią
2 kubitų supynimą ir jų matavimus.

![](./media/image9_6.png)

**9.6 pav.** Klaidoms neatspari loginė grandinė, atliekanti kubitų supynimą ir
jų matavimą

![](./media/image9_7.png)

**9.7 pav.** Loginė grandinė, dviem 3 kubitų kodo loginiams kubitams atliekanti
klaidoms atsparius Hadamardo ir sąlyginius $cX$ loginius vartus

9.7 pav. pateikiame klaidoms atsparius
$\bar{H}$ ir $\overline{CNOT}$ loginius elementus (žymimi su
...
vartai $\overline{CNOT}$, veikiantys tarp atskirų kubitų kiekviename
bloke, užtikrina, kad klaida $H$ $k_1$ bloke paveiks daugiausiai vieną
papildomą kubitą $k_2$ bloke. Tą patį galima pasakyti ir apie pačius
$\overline{CNOT}$, kurių klaidingas atlikimas paveikia tik po vieną
...
**9.8 pav.** Iliustracijoje 9.6 pav. parodytos loginės grandinės klaidoms
atspari versija. Du loginiai kubitai čia bendrai koduojami $n$ kubitais, toliau
patikrinama, ar koduojant neįvyko klaidų, ir atitinkamai atliekamas taisymas.
Pirmam loginiam kubitui pritaikomi klaidoms atsparūs $H$; matomi dar du
periodiniai klaidų nustatymai ir 2 loginių kubitų klaidoms atsparūs $cX$;
galiausiai atliekami klaidoms atsparūs kubitų būsenos matavimai

Klaidoms atspari kvantinė grandinė
pakeičia loginius vartus $H$ ir $CNOT$, taip pat matavimų operacijas
klaidoms atspariais loginiais elementais. Be to, loginių kubitų
$\overline{|0\rangle}_L$ paruošimas turi būti taip
...
Toliau aptarkime, kaip realizuoti klaidoms atsparų būsenų matavimą. Jam
taikome Hadamardo testu pagrįstą minėtą kodo būsenų paruošimo metodą
(žr. 9.9 poskyrį), kuris čia pritaikomas 3 kubitų būsenos
$|\psi\rangle$ matavimui. Šiuo metodu galima atlikti
loginio Pauli-$Z$ ar bet kokio kito $n$ kubitų ermitinio operatoriaus
$U$, išreiškiamo skersiniais loginiais vartais, projekcinį matavimą į jo
+1 ir -1 poerdvius. Tai atliekanti kvantinė grandinė pateikta 9.9 pav.
...
atsparus, nes naudojami klaidoms neatsparūs $H$ ir $CNOT$ loginiai
...
**9.9 pav.** Klaidoms atsparūs 3 kubitų loginio kubito būsenos $|\psi\rangle$
bendrojo tipo matavimai
...
**9.10 pav.** Kodų konkatenacijos schema. Nurodytos nepataisomos klaidos
tikimybė kiekviename kodavimo lygyje
...
eksponentės $2^k$ klaidingo atsakymo tikimybė gali būti padaroma
...
\begin{equation}
k \geq \left\{1 + \frac{\log\left\lbrack\frac{p(n)}{\varepsilon}\right\rbrack}{
\log\left\lbrack\frac{1}{cp}\right\rbrack}\right\}\,.
\end{equation}
...
klaidų šaltinius. Tačiau didėjant kubitų skaičiui ir mažėjant šioms
...
įvykdyti kvadratinę $2 \times 2$ grandinę, sudarytą iš 2 gylio su pasirinktais
...
randamas procesoriaus kvantinis tūris, jis žymimas simboliu $V_{Q}$, o
...
protokolą atliekanti grandinė yra parodyta 9.11 pav.

![](./media/image9_11.png)

**9.11 pav.** Kvadratinėmis $(d \times d)$ grandinėmis pagrįstas kvantinio tūrio
...
$|x\rangle$ aukščiau medianos yra \~85 % ir
asimptotiškai artėja prie 50 % tikimybės, jeigu įrenginys veikia itin
...
Siekdami iliustruoti kubitų tarpusavio jungimo įtaką skaičiavimams, 9.12
pav. pateikiame dvi skirtingas kvantinio procesoriaus kubitų jungčių
schemas. Pirmoje (9.12 pav., a) pavaizduota IBM superlaidininkais
...
*CNOT*. Pavyzdžiui, atlikti *CNOT* tarp kubitų #1 ir #15 tiesiogiai
...
Antroji (9.12 pav., b) pateikta kvantinio procesoriaus architektūra
...
**9.12 pav.** Dvi kvantinio procesoriaus architektūros, pasitelkiančios
skirtingą kubitų tarpusavio jungimą. Tai nulemia, tarp kurių kubitų galima
atlikti 2 ar daugiau kubitų sąlyginius loginius vartus
